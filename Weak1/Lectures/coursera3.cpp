...
{ //работа с контейнерами (тоесть типами которые хранят больше чем 1 значение)
   ...
   for(char c : s) { //для (чего-то ИЗ(:) чего-то)
...
{
   vector<int> nums = {1, 2, 3, 5, 6, 3, 7, 9, 5, 0, 4};
   int quantity = count(begin(nums), end(nums), 5); //функция count
   cout << "Здесь " << quantity << "пятёрок\n";
   return 0;
}
...
/*ИТОГ:
 *Познакомился с библиотекой algorithm
 *Познакомился с готовыми алгоримами sort и count
 */